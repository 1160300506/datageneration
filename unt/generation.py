...
if __name__ == '__main__':
    ...
    random.seed(10)
    factor = 1280
    users_num = 1000 * factor  # 生成的用户数量
    users_inform_table = user_inform_generate(users_num)  # 生成用户基本信息
    user_behavior_generate(users_num, 10000 * factor, 3, users_inform_table)  # 生成1000000条用户行为记录
    dox_l = dox_generate(10000 * factor, users_num, users_inform_table)  # 生成博文
    user_password_generate(users_num)  # 生成密码
    user_thumb_generate(users_num, 10000 * factor, users_inform_table)  # 生成点赞统计
    graph_generate(users_num, users_inform_table, dox_l)  # 生成大图数据
    end = time.time()
    print("finish generate data! costs time %f" % (end - start))