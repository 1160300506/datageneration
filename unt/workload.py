...
def workload1(mode=0):
    ...
    for i in range(0, len(topic_list)):
        for j in range(0, len(blog_v)):
            count += 1
            sql = "Select activity.device, count(activity.device) as device_count where log_time between %s " \
                  "and %s and uid in (Select gra.s join gra (s = a.s, o = a.o, p = a.p, bp = b.p, " \
                  "bo = b.o, cp = c.p, co = c.o, dp = d.p, do = d.o) " \
                  "a, b, c, d a.s = b.s and b.o = c.s and a.s = d.s where " \
                  "gra.p = 'follow' and gra.o = '%s' and gra.bp = 'publish' and gra.cp = 'topic' and gra.co " \
                  "= '%s' and gra.dp = 'gender' and gra.do = 'female') " \
                  "group by device;" % ("20200701", "20200707", blog_v[j], topic_list[i])
            f.write(sql + "\n")

            sql_graph = "Select a.s from graph a join graph b on a.s = b.s join graph c on b.o = c.s " \
                        "join graph d on a.s = d.s " \
                        "where a.p = 'follow' and a.o = '%s' and b.p = 'publish' and c.p = 'topic' and c.o = '%s' " \
                            "and d.p = 'gender' and d.o = 'female';" % (
            ...
            if mode == 1:
                cql_relation = "Select device from activity where" \
                               " log_time >= '%s' and log_time <= '%s' and basicinfo.id in (graph_answer.id)" \
    ...
    #     if dox_l[i][2] == "the Internet":
    #         uid.append(dox_l[i][1])
    # print(len(uid))
    # uidnew1 = []
    # print("2. 在女性用户中")
    # for i in range(0, len(uid)):
    #     if user_inform_table[int(uid[i])-1][3] == "1":
                ...
                if user_behavior_table[j][2] not in data:
                    ...
                    if mode == 0:
                        sql_relation = "Select basicinfo.id from basicinfo join activity on basicinfo.id = activity.uid" \
                                       " where basicinfo.register_date < %s0101 and basicinfo.id in (graph_answer.id) and" \
                                       " activity.log_time = %s;" % (str(i), user_behavior_table[j][2])
                        f_relation_default.write(sql_relation + "\n")
                    if mode == 1:
                        sql_relation = "select id from basicinfo where basicinfo.id in (graph_answer.id)" \
                                       " and register_date < %s0101;" %str(i)
                        cql_relation = "select uid from activity where basicinfo.id in (graph_answer.id)" \
                    ...
                    if mode == 2:
                        cql_relation = "select uid from activity where basicinfo.id in (graph_answer.id)" \
                                       ...
                                       user_behavior_table[j][2][6], user_behavior_table[j][2][7])
                        f_relation_artificial.write(cql_relation + "\n")
                        cql_relation = "select id from basicinfo where basicinfo.id in (graph_answer.id) and register_date = '%s-01-01' allow filtering;" %str(i)
                        f_relation_artificial.write(cql_relation + "\n")
...
def workload3(mode=0):
    ...
    for i in range(2012, 2017):
        ...
        if mode == 0:
            sql_relation = "Select basicinfo.id from basicinfo join activity on basicinfo.id = activity.uid" \
                           " where basicinfo.register_date < %s0101 and basicinfo.id in (document_answer.id) and" \
                           " activity.log_time >= 20200701;" % (str(i))
            f_relation_default.write(sql_relation + "\n")
        if mode == 1:
            cql_relation = "select uid from activity where basicinfo.id in (document_answer.id)" \
                           " and log_time >= '2020-07-01' allow filtering;"
            sql_relation = "select id from basicinfo where basicinfo.id in (document_answer.id)" \
                           " and register_date < %s0101;" % str(i)
            f_relation_intelligence_sql.write(sql_relation + "\n")
            f_relation_intelligence_cql.write(cql_relation + "\n")
        if mode == 2:
            cql_relation = "select id from basicinfo where basicinfo.id in (document_answer.id) and register_date = '%s-01-01' allow filtering;" % str(
                i)
            f_relation_artificial.write(cql_relation + "\n")
            cql_relation = "select uid from activity where basicinfo.id in (document_answer.id)" \
                           " and log_time >= '2020-07-01' allow filtering;"
                ...
                for k in range(0, len(type_user)):
                    for z in range(2010, 2021):
                        mysql = "Select rel.* where rel.id in (Select gra.s, gra.do join gra (s = a.s, o = a.o, " \
                                "p = a.p, bp = b.p, bo = b.o, cp = c.p, co = c.o) " \
                                "a, b, c, d a.s = b.s and a.s = c.s where " \
                                "gra.p = 'follow' and gra.o = '%s' and gra.bp = 'position' " \
                                "and gra.bo = '%s' gra.cp = 'type' and gra.co = '%s') " \
                        ...
                        mysql_graph = "Select a.s from graph a join graph b on a.s = b.s join graph c on" \
                                      ...
                                      "and b.p = 'position' and b.o = '%s' and c.p = 'type' and c.o = '%s';" % (blog_v[i], occupation_list[j], type_user[k])
                        sql_relation = ""
                        if mode == 0:
                            sql_relation = "Select * from basicinfo where " \
                                           "basicinfo.id in (graph_answer.id) and basicinfo.register_date < %s0101;" % str(z)
                            f_relation_default.write(sql_relation + "\n")
                        if mode == 1:
                            sql_relation = "Select * from basicinfo where " \
                                           "basicinfo.id in (graph_answer.id) and basicinfo.register_date < %s0101;" % str(z)
                            f_relation_intelligence_sql.write(sql_relation + "\n")
                        if mode == 2:
                            cql_relation = "select * from basicinfo where basicinfo.id in (graph_answer.id)" \
    ...
    for i in range(0, len(c)):
        ...
        if mode == 2:
            cql_relation = "select id from basicinfo where age >= 20 and age <= 30" \
                           " and basicinfo.id in (document_answer.id) allow filtering;"
            f_relation_artificial.write(cql_relation + "\n")
        mysql_kv = "Select * from userlike where userlike.uid in relation_answer.id;"
        f_dox.write(mysql_dox + "\n")
        f_kv.write(mysql_kv + "\n")
...
def workload6(mode=0):
    ...
    topic_list = ["University", "School", "Food", "Campus", "Travel", "Harbin", "Car", "Agriculture", "Star",
                  "Computer", "Medical", "Beauty", " Department Store",
                  "College Entrance Examination", "Pneumonia", "Energy", "Military", "Graduation", "Beijing", "Hebei",
                  "School Celebration", "Beauty", "Tiktok", "Wuhan", "USA", "the Internet"]
...
if __name__ == '__main__':
    ...
    # for i in range(0, len(uidnew2)):
    #     for j in range(0, len(follow)):
    #         if follow[j][0] == uidnew2[i] and follow[j][2] == '04078':
    #             made.append(uidnew2[i])
    #             flag = flag + 1
    #
    # print(made)
    ...
    # for i in range(0, len(uid)):
    #     for j in range(0, len(inform)):
    #         if inform[j][0] == uid[i] and inform[j][4] == "student":
    #             uidnew.append(uid[i])
    # print(len(uidnew))
    #
    # sen = read_senior_user()
