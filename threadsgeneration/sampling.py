...
def extract1(filename, h):
    ...
    for line in f:
        ...
        if c in h:
            ...
            variable1 = line.split(" ")[89]
            variable2 = line.split(" ")[100]
            graph_sql = "Select a.s, e.o, f.o from graph a join graph b on a.s = b.s join graph c on b.o = c.s " \
                        ...
                        "and d.p = 'gender' and d.o = 'female' and e.p = 'occupation' and f.p = 'name';" % (variable1, variable2)
    ...
    for line in f:
        ...
        if c in h:
            ...
            relation_sql = "Select basicinfo.id from basicinfo join activity on basicinfo.id = activity.uid" \
                                       " where basicinfo.register_date < %s and uid >= 101 and uid <= 105" \
...
def extract4(filename, h):
    ...
    for line in f:
        ...
        if c in h:
            f_sampling.write(line)
            variable1 = line.split(" ")[59]
            variable2 = line.split(" ")[67]
            variable3 = line.split(" ")[74] + " " + line.split(" ")[75]
            variable4 = line.split(" ")[83][:-1]
            graph_sql = "Select a.s, d.o from graph a join graph b on a.s = b.s join graph c on" \
                          " a.s = c.s join graph d on a.s = d.s where a.p = 'follow' and a.o = %s " \
                          "and b.p = 'position' and b.o = %s and c.p = 'type' and c.o = %s " \
                          "and d.p = 'name';" % (variable1, variable2, variable3)
            relation_sql = "Select * from basicinfo where id " \
                           ">= 101 and id <= 105 and basicinfo.register_date < %s;" % variable4
            f_sampling_graph.write(graph_sql + "\n")
            f_sampling_relation.write(relation_sql + "\n")
    f.close()
...
if __name__ == '__main__':
    ...
    #         " and gra.bp = 'comment' and gra.cp = 'topic' and gra.co = '%s' )"
    #
    #
    # print( mysql.split(" ").index("'%s'"))