...
def user_inform_generate(user_num):
    ...
    """
    print("[INFO:主线程]开始生成"+str(user_num) + "个用户基本信息...")
...
def user_behavior_generate(user_num, user_behavior_num, user_device_max, user_inform_table):
    """
        ...
        """
    print("[INFO:Thread1]开始生成"+str(user_behavior_num)+"条用户登录行为信息...")
...
def dox_generate(dox_num, user_num, user_inform_table):
    """
        ...
        并将数据写入blog_dox.json
        """
    print("[INFO:Thread2]开始生成" + str(dox_num) + "条博文文档信息...")
    ...
    for i in range(0, dox_num):
        ...
        blog_content = ''.join(choices(string.ascii_letters + string.digits, k=random.randint(30, 100)))
...
# 生成点赞统计kv数据
def user_thumb_generate(user_num, thumb_num):
    """
        args:
              users_num: 用户的个数。
            ...
            """
    print("[INFO:Thread3]生成"+str(thumb_num)+"条点赞数据...")
    thumb_statistics_kv = {}
    uid_len = len(str(user_num))
...
if __name__ == '__main__':
    ...
    # 开始创建线程
    threads = []
    t_1 = threading.Thread(target=user_behavior_generate, args=(users_num, 100000 * factor, 3, users_inform_table))
    t_2 = MyThread(dox_generate, args=(100000 * factor, users_num, users_inform_table))
    t_3 = threading.Thread(target=user_thumb_generate, args=(users_num, 100000 * factor))
    threads.append(t_1)
    threads.append(t_2)
    threads.append(t_3)
    for t in threads:
    ...
    graph_generate(users_num, users_inform_table, t_2.get_result())  # 生成大图数据
    end = time.time()
    print("finish generate data! costs time: %f" % ((end - start) / 60))
